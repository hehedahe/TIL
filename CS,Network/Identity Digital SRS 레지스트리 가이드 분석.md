...
### **4.4 Redemption Grace Period(복구 유예 기간)**
  
Identity Digital SRS RGP 프로세스는 다른 레지스트리들도 사용하는 업계 표준을 따른다.
RGP는 자동화된 복구 프로세스를 통해 제공되고, 레지스트라는 복구 폼을 작성해야 한다.  
...
도메인이 30일 뒤에 복구되지 않으면 5일 동안 도메인을 생성할 수 있는 상태로 변경이 불가능한 삭제 보류 기간이 된다. 이 기간이 끝나면 도메인은 삭제될 것이고, 사용가능한 도메인 pool에 릴리즈될 것이다. RGP 기간(30일)과 RHP(5일)에 포함되는 도메인 목록은 모든 레지스트
