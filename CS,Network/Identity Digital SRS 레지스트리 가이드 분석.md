...
- AGP 중 도메인을 삭제하면 레지스트라는 등록 수수료를 환급해 준다.

도메인은 Identity Digital SRS DB에서 삭제되고 다른 레지스트라에서 즉시 등록이 가능하다.

- 5일의 유예 기간이 지나고 삭제하는 경우:

- 30일 동안 복구 기간(redemptionPeriod) 상태가 된다. 이 때 restore하거나 delete할 수 있다.
...
- AGP 내 도메인이 갱신되면 생성 트랜잭션에 대해 유예 기간 환급이 주어지지 않는다.

- 초기 등록 요금 외에도 레지스트라에 등록된 크레딧은 갱신된 연수 만큼 인출된다.
...
- 이 기간 내 도메인을 삭제하면 레지스트라는 도메인이 갱신된 연수에 따라 갱신 수수료를 환급해 준다.

- 그러고 나서 Add Grace Period 기간 동안 도메인을 삭제하지 않으면 Redemption Grace Period(복구 유예 기간)에 들어가게 된다.
...
- 도메인은 총 10년까지 Renew Grace Period 동안 갱신될 수 있다. 이 상태에서 유예 기간 환불을 발생하지 않는다.

- 도메인 만료일은 최대 10년으로 제한된다.

- 레지스트라에 등록된 크레딧은 갱신된 연수 만큼 인출된다.
...
- 이 기간 내 도메인이 이전되면 추가된 갱신 연수는 제거되지 않고, 유예 기간 환불도 진행되지 않는다.
...
#### **1. 갱신**
- 이 기간 동안 도메인을 갱신하면 이전에 대한 환급은 제공되지 않는다. 이전 요금에 레지스트라에 사용가능한 크레딧은 등록된 갱신 연수만큼 
